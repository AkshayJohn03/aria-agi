...
def build_loaders(tokenized_dataset_path, batch_size, num_workers, pin_memory=False):
    ...
    # Collate function to convert list of dictionaries (from datasets.Dataset) to PyTorch tensors
    def collate_fn(batch: List[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
        """Converts a batch of dataset items into PyTorch Tensors."""
        input_ids = torch.tensor([item["input_ids"] for item in batch], dtype=torch.long)
        labels = torch.tensor([item["labels"] for item in batch], dtype=torch.long)
        # NOTE: If attention masks were needed, they would be calculated here based on padding.
        # However, for simplicity and next-token prediction with -1 label masking, 
        # we rely on the labels to implicitly handle masking.
...
def evaluate(model: nn.Module, val_loader: DataLoader, config: V3Config, device: torch.device) -> Tuple[Dict[str, float], float]:
    """Runs a single pass over the validation set."""
    model.eval()
    ...
    for batch in pbar_eval:
        try:
            ...
            num_batches += 1
            pbar_eval.set_postfix(avg_loss=total_loss/num_batches)
    ...
    try:
        while step < config.max_steps:
            ...
            if step % config.eval_every == 0:
                ...
                print(f"[{time.strftime('%H:%M:%S')}] [EVAL] Step {step} | Val Loss: {current_val_loss:.4f} | Val PPL: {metrics['val/ppl']:.2f}")
