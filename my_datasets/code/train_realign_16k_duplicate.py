...
data_glob = "datasets/processed/test_openorca/train"
output_dir = "artifacts/zia_ift_v4_longctx"
resume_ckpt = os.path.join(output_dir, "checkpoints", "checkpoint_step5955.pt")  # change if needed

# model hyperparams
d_model, n_layers, n_heads, mlp_ratio, dropout = 384, 8, 6, 4, 0.1

# training hyperparams; adjust as you like
desired_context_len = 32768 #16384    # target context for this run (will expand if ckpt smaller)
lr = 2e-5  #8.0e-05                  # lower LR for stable fine-tuning after expansion
weight_decay = 0.01
batch_size = 4
grad_accum = 112
autosave_minutes = 30
...
# scheduler params
scheduler_tmax = 4000         # number of optimizer steps for cosine period
...
def train():
    ...
    if target_ctx > base_ctx:
        tqdm.write(f"[⚙️] Expanding context from {base_ctx} → {target_ctx}")
