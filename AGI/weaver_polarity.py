...
print(f"🧲 PROJECT WEAVER: POLARITY PROTOCOL (Antipodal Targets) ON {DEVICE}")
...
# --- THE GEOMETRY FIX ---
# Instead of random orthogonal targets, we force them to be ANTIPODAL.
# Beach = +1 direction, Sauna = -1 direction.
# This makes averaging them result in 0 (Failure).
v_beach = get_vector("beach")
v_sauna = -v_beach # <--- THE POLARITY FORCE
...
for gen in range(600):
    ...
    # --- POLARITY FITNESS ---
    # Task A Target: +1 (Beach)
    sim_a = F.cosine_similarity(out_a.unsqueeze(0), v_beach.unsqueeze(0)).item()
    
    # Task B Target: +1 (Sauna) 
    # Note: v_sauna is -v_beach. So we compare to v_sauna.
    sim_b = F.cosine_similarity(out_b.unsqueeze(0), v_sauna.unsqueeze(0)).item()
    
    avg_fitness = (sim_a + sim_b) / 2
    ...
    if gen % 50 == 0:
        print(f"Gen {gen}: Summer->Beach: {sim_a:.2f} | Winter->Sauna: {sim_b:.2f} | Avg: {avg_fitness:.3f}")

print(f"🏆 POLARITY LOGIC TRAINED. Best Fitness: {best_fitness:.3f}")
...
# Ideally, similarity to Sauna (-Beach) should be negative
...
print(f"Context: WINTER -> Sauna: {s_sauna_w:.2f} | Beach: {s_beach_w:.2f}")

# VERDICT
# If it worked, Summer->Beach should be +X, Summer->Sauna should be -X
# And Winter->Sauna should be +Y, Winter->Beach should be -Y
gap = (s_beach - s_sauna) + (s_sauna_w - s_beach_w)
if gap > 1.0: 
    print(f"\n✅ SUCCESS: POLARITY SWITCHED. (Total Gap: {gap:.2f})")
else:
    print(f"\n⚠️ FAILURE: Still Muddled. (Total Gap: {gap:.2f})")