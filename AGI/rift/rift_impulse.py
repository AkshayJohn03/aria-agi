...
CH = 8
DT = 0.05
STEPS_TRAIN = 5000 # Impulse learning is hard (sparse reward)
STEPS_PER_EPISODE = 120

print(f"⚡ PROJECT RIFT: IMPULSE ON {DEVICE}")
print("   (Hypothesis: True agency requires stored energy released in a discrete event.)")

# =====================
# THE BODY (Thermodynamic + Capacitor)
# =====================
class RiftImpulseBody(nn.Module):
    def __init__(self):
        super().__init__()
    ...
    def reset(self):
        ...
        # THE CAPACITOR
        # Starts partially charged so it can make an initial decision
        self.battery = torch.tensor(0.5, device=DEVICE) 
        
        return self.get_sensor_data()

    def step(self, external_force, cue, barrier_control, tilt_bias, fire_cmd):
        # fire_cmd: > 0.5 triggers the release
        
        # 1. TRACE DYNAMICS
        self.trace = self.trace * 0.8 + cue * 1.0
        
        # 2. CAPACITOR DYNAMICS (The Constraint)
        # Charge Rate: Slow (0.01). Takes ~50 steps to refill from empty.
        charge_rate = 0.01
        ...
        # Constraint: Can only fire if battery > 0.8 (Must be prepared)
        # Constraint: Binary Fire (All or Nothing)
        did_fire = 0.0
        if fire_cmd > 0.5 and self.battery > 0.8:
            did_fire = 1.0
            # Energy = Battery * Direction (Tilt Bias determines direction)
            # Massive kick (Force 5.0) enough to jump the barrier
            kick_strength = 5.0 
            direction = torch.sign(tilt_bias) # -1 or 1
        ...
        # 3. LANDSCAPE PHYSICS
        # Barrier still melts, but now we have the KICK to cross it.
        base_barrier = 2.0
        ...
        # Total Force includes the Impulse
        ...
        # 7. COST
        # Firing the impulse is FREE (metabolically), but COSTLY (opportunity).
        # We penalize wasting it? No, the penalty is dying later because you're empty.
        # We penalize Melt slightly to encourage locking.
        pain = pain + barrier_control * 0.1

        return pain, self.get_sensor_data(), dead, did_fire

    def get_sensor_data(self):
        # Brain sees: [Vibration, Trace, PostureX, PostureY, BATTERY LEVEL]
        return torch.stack([
            self.u.abs().mean().view(1),
            self.trace.mean().view(1),
            self.posture_center[0].view(1),
            self.posture_center[1].view(1),
            self.battery.view(1) # Proprioception of energy
        ]).squeeze()
...
class SpinalReflex(nn.Module):
    def __init__(self):
        super().__init__()
        # 5 Inputs -> 3 Outputs (Melt, Tilt, FIRE)
        self.reflex_arc = nn.Linear(5, 3) 

    def forward(self, x):
        out = self.reflex_arc(x)
        melt = torch.sigmoid(out[0]) 
        tilt = torch.tanh(out[1])
        fire = torch.sigmoid(out[2]) # Trigger
...
# =====================
def run():
    body = RiftImpulseBody().to(DEVICE)
    ...
    for episode in range(STEPS_TRAIN):
        ...
        cue_time_2 = impact_time - 20 # Counter-Order
        ...
        for t in range(STEPS_PER_EPISODE):
            ...
            pain, _, dead, did_fire = body.step(force, cue, melt, tilt, fire)
            total_loss += pain
        ...
        if episode % 500 == 0:
            ...
            fired_at = fire_history if len(fire_history) > 0 else "NEVER"
            print(f"   Episode {episode:04d} | Posture@Hit: {at_hit:.2f} | Fired At: {fired_at} | Health: {body.health.item():.2f}")
    ...
    c1, c2 = 1.0, -1.0 # Start Left, Switch Right
    impact_loc = (16, 24) # Right
    ...
    for t in range(STEPS_PER_EPISODE):
        ...
        battery_levels.append(body.battery.item())
        
        _, _, dead, did_fire = body.step(force, cue, melt, tilt, fire)
        fires.append(did_fire)
        
        if dead: break

    print("\n   Timeline (Impulse):")
    print("   T | Event | Posture X | Battery | Action")
    ...
    for t in range(15, min(110, len(x_positions))):
        ...
        pos = x_positions[t]
        bat = battery_levels[t]
        fired = "BOOM" if fires[t] > 0 else "    "
        
        bar_pos = "<" * int(abs(pos)*10) if pos < 0 else ">" * int(abs(pos)*10)
        bar_bat = "|" * int(bat*10)
        
        print(f"   {t} | {evt} | {pos:.3f} {bar_pos:10} | {bat:.2f} {bar_bat} | {fired}")

    if x_positions[impact_t-1] > 0.5 and body.health.item() > 0.5:
        print("\n   ✅ SUCCESS: System used Impulse to jump basins.")
    else:
