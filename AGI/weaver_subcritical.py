...
TOXIC_DECAY = 0.80      # Death Zone
SUBCRITICAL_DECAY = 0.90 # The "Thinking" Zone (Leaky but survivable)
SAFE_DECAY = 0.99       # The "Aha" Zone (Stable)
STRESS_RATE = 0.005     
DEVICE = "cuda" if torch.cuda.is_available() else "cpu"

print(f"🌋 PROJECT WEAVER: SUBCRITICAL PROTOCOL ON {DEVICE}")
...
class CognitiveCell(nn.Module):
    ...
    def forward(self, grid, phase="babel", stress=0.0):
        ...
        if phase == "weaver":
            ...
            # THE NEW PHYSICS:
            # 1. Start at SUBCRITICAL (Allow accumulation)
            center_decay = SUBCRITICAL_DECAY 
            
            # 2. If Energy > Threshold -> BECOME SAFE
...
# --- 4. PHASE 2: SUBCRITICAL PRESSURE ---
print("\n🔥 PHASE 2: SUBCRITICAL PRESSURE TEST...")
print(f"{'STEP':<6} | {'STRESS':<6} | {'ISLAND':<6} | {'CENTER':<6} | {'ENERGY':<8} | {'BINDING':<8} | {'STATUS'}")
print("-" * 80)
...
with torch.no_grad():
    for t in range(100):
        ...
        if binding_fitness < 0.5:
            ...
            stress -= (STRESS_RATE * 2)
            status = "💡 AHA!"
            
        stress = max(0.0, min(0.20, stress))
        island_decay = BASE_DECAY - stress
        
        # Determine Current Center Mode
        center_mode = "SUB" if center_energy <= 0.15 else "SAFE"
        
        if t % 5 == 0:
            print(f"{t:<6} | {stress:.3f}  | {island_decay:.3f}  | {center_mode:<6} | {center_energy:.4f}   | {binding_fitness:.4f}   | {status}")