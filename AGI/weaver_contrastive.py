...
print(f"⚖️ PROJECT WEAVER: CONTRASTIVE LOGIC PROTOCOL ON {DEVICE}")
...
class CognitiveCell(nn.Module):
    ...
    def forward(self, grid, prev_grid=None, sniper_active=False):
        ...
        # --- PHASED SNIPER ---
        delta = 0.0
        if prev_grid is not None and sniper_active:
            cy, cx = h // 2, w // 2
...
for gen in range(600): # More gens for harder task
    # Mutate
    with torch.no_grad():
        mutant.w_perceive.weight.add_(torch.randn_like(mutant.w_perceive.weight) * MUTATION_RATE)
    ...
    # --- METRICS ---
    # Task A: Summer -> Beach (Target), Sauna (Distractor)
    sim_a_target = F.cosine_similarity(out_a.unsqueeze(0), v_beach.unsqueeze(0)).item()
    sim_a_distractor = F.cosine_similarity(out_a.unsqueeze(0), v_sauna.unsqueeze(0)).item()
    
    # Task B: Winter -> Sauna (Target), Beach (Distractor)
    sim_b_target = F.cosine_similarity(out_b.unsqueeze(0), v_sauna.unsqueeze(0)).item()
    sim_b_distractor = F.cosine_similarity(out_b.unsqueeze(0), v_beach.unsqueeze(0)).item()
    
    # --- CONTRASTIVE FITNESS FUNCTION ---
    # We want Target HIGH and Distractor LOW.
    # Score = Target - 0.5 * Distractor (0.5 is a damper to prevent instability)
    score_a = sim_a_target - (0.5 * sim_a_distractor)
    score_b = sim_b_target - (0.5 * sim_b_distractor)
    
    avg_fitness = (score_a + score_b) / 2
    ...
    if gen % 50 == 0:
        print(f"Gen {gen}:")
        print(f"  Summer: Beach={sim_a_target:.2f} | Sauna={sim_a_distractor:.2f} (Gap: {sim_a_target-sim_a_distractor:.2f})")
        print(f"  Winter: Sauna={sim_b_target:.2f} | Beach={sim_b_distractor:.2f} (Gap: {sim_b_target-sim_b_distractor:.2f})")

print(f"🏆 CONTRASTIVE LOGIC TRAINED. Best Fitness: {best_fitness:.3f}")
...
print(f"Context: SUMMER -> Beach: {s_beach:.2f} | Sauna: {s_sauna:.2f} | GAP: {s_beach - s_sauna:.2f}")
...
print(f"Context: WINTER -> Sauna: {s_sauna_w:.2f} | Beach: {s_beach_w:.2f} | GAP: {s_sauna_w - s_beach_w:.2f}")