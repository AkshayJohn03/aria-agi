...
print("🧬 PROJECT RIFT: SPECIATION v2 ON", DEVICE)
print("   (Hypothesis: Suffering drives adaptation. Damage -> Health, Frustration -> Battery.)")
...
class RiftBody:
    ...
    def reset(self):
        ...
        # Evolution Trackers
        self.peak_damage = 0.0
        self.peak_dissonance = 0.0
    ...
    def step(self, force, cue, melt, tilt, fire):
        ...
        # Track Dissonance (Frustration)
        dissonance = 0.0
        if abs(belief) > 0.1 and belief * self.pos[0].item() < 0:
            dissonance = 0.5
            self.stability = max(0.0, self.stability - 0.04)
            self.peak_dissonance = max(self.peak_dissonance, dissonance)
        else:
            self.stability = min(1.0, self.stability + 0.01)

        self.battery = min(self.battery + 0.02, self.battery_max)

        # TRADEOFFS (The Laws of Burden)
        # Mass increases with Health (Harder to move)
        mass = 1.0 + 0.6 * (self.health_max - 1.0)
        # Damping increases with Battery (Sluggish)
        damping = 0.5 + 0.3 * (self.battery_max - 1.0) + (1.0-self.stability)

        impulse = 0.0
        if fire > 0.5 and self.battery > 0.6 * self.battery_max:
            # Kick must overcome Mass
        ...
        if deform > 0.1:
            dmg = deform * 0.4
            self.health -= dmg
            self.peak_damage = max(self.peak_damage, dmg) # Track trauma

        return self.health <= 0, dmg

    def evolve(self):
        # TRAUMA-DRIVEN EVOLUTION
        # If I took damage -> Tougher skin needed -> Health++
        if self.peak_damage > 0.01:
            self.health_max += 0.05

        # If I felt stuck (Dissonance) -> More agency needed -> Battery++
        if self.peak_dissonance > 0.1:
            self.battery_max += 0.05

        # Atrophy (Use it or lose it)
        if self.peak_damage < 0.001 and self.health_max > 1.0:
            self.health_max -= 0.01
        if self.peak_dissonance < 0.001 and self.battery_max > 1.0:
