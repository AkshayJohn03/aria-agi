...
print(f"🧠 MNEMOS-P5: DISSIPATION & PREDICTION ON {DEVICE}")
print("   (Hypothesis: Learned conductive paths allow trauma energy to vent, reducing peak damage.)")
...
class Substrate(nn.Module):
    def __init__(self):
        ...
        # PARAMETERS
        # Structure: Base Stiffness = 1.0 (Glass, not Cloud)
        self.k_structure = nn.Parameter(torch.ones(1, CH, GRID, GRID, device=DEVICE) * 1.0)

        # Conductivity: Starts LOW (0.01)
        self.conductivity = nn.Parameter(torch.ones(1, CH, GRID, GRID, device=DEVICE) * 0.01)
    ...
    def step(self, external_force, mode="learn"):
        ...
        # Damage Accumulator (for metric)
        # Threshold 0.5 (Fragile)
        current_damage = (stress_load > 0.5).float().sum().item()

        # LEARNING
        with torch.no_grad():
            if mode == "learn":
                # 1. SCARRING
                fracture = (stress_load > 0.5).float()
                # Harden slightly where broken
                self.k_structure.data += fracture * 0.02
                self.k_structure.data.clamp_(1.0, 5.0)

                # 2. HEBBIAN EROSION (Wire the Path)
                neighbor_activity = F.avg_pool2d(vibration, 5, 1, 2) # Wider pool to catch wave motion
                coincidence = vibration * neighbor_activity
                self.conductivity.data += coincidence * 0.01
                self.conductivity.data.clamp_(0.01, 2.0)

        return self.u, current_damage
...
def run():
    ...
    for t in range(STEPS_TRAIN):
        ...
        # IMPACT at t=35
        if t % 80 == 35:
            force[:,:,imp_loc[0]-2:imp_loc[0]+2, imp_loc[1]-2:imp_loc[1]+2] = 10.0
        ...
        if t % 160 == 0:
            ...
            print(f"   Step {t:03d} | Path Conductivity: {path_quality:.4f} | Damage: {damage:.1f}")
    ...
    for t in range(100):
        force = torch.zeros_like(system.u)
        if t == 35: force[:,:,imp_loc[0]-2:imp_loc[0]+2, imp_loc[1]-2:imp_loc[1]+2] = 10.0

        u, d = system.step(force, mode="test")

        # Measure Peak Stress at Impact Site
        local_stress = (system.k_structure * system.u)[:,:,imp_loc[0]-2:imp_loc[0]+2, imp_loc[1]-2:imp_loc[1]+2].abs().mean().item()
    ...
    for t in range(100):
        force = torch.zeros_like(system.u)
        if t == 10: force[:,:,cue_loc[0]-2:cue_loc[0]+2, cue_loc[1]-2:cue_loc[1]+2] = 5.0
        if t == 35: force[:,:,imp_loc[0]-2:imp_loc[0]+2, imp_loc[1]-2:imp_loc[1]+2] = 10.0
