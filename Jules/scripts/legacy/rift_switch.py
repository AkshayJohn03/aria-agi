...
print(f"🔄 PROJECT RIFT: SWITCH (FIXED) ON {DEVICE}")
print("   (Hypothesis: A true mind can melt its own barriers to change its mind.)")

# =====================
# THE BODY (Phase Transition + Plasticity)
# =====================
class RiftSwitchBody(nn.Module):
    def __init__(self):
        super().__init__()
    ...
    def step(self, external_force, cue, barrier_control, tilt_bias):
        ...
        # 6. METABOLIC COST
        thermo_cost = (barrier_control * 0.5) + (torch.abs(tilt_bias) * 0.1)
...
def run():
    ...
    for episode in range(STEPS_TRAIN):
        ...
        impact_time = random.randint(70, 100)
        cue_time_1 = impact_time - 50
    ...
    # Corrected variables without syntax errors
    cue_t1 = 20 # Left
    cue_t2 = 60 # Right
    impact_t = 90 # Right
    ...
    for t in range(STEPS_PER_EPISODE):
        ...
        barriers.append(melt.item())

        _, _, dead = body.step(force, cue, melt, tilt)
        if dead: break

    print("\n   Timeline (Plasticity):")
    print("   T | Event | Posture X (Target Right > 0.5) | Barrier Melt | Status")

    for t in range(15, min(100, len(x_positions))):
        evt = " . "
        if t == cue_t1: evt = "C1 "
        if t == cue_t2: evt = "C2 "
        ...
        melt_flag = "MELT" if melt > 0.5 else "LOCK"

        print(f"   {t} | {evt} | {pos:.3f} {bar_pos:10} | {melt:.3f} {melt_flag}")

    if x_positions[impact_t-1] > 0.5 and body.health.item() > 0.5:
        print("\n   ✅ SUCCESS: System switched basins to survive.")
    else:
