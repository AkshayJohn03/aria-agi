...
print(f"🌋 RIFT EXPERIMENT HARNESS v3 (Fixed) ON {DEVICE}")
...
def input_stress_pattern(t):
    ...
    if t % 50 == 0: # The Event lasts 10 steps
        cy, cx = GRID_SIZE//2, GRID_SIZE//2
        force[:,:,cy-4:cy+4,cx-4:cx+4] = 5.0
    return force
...
def mutate_topology(plate):
    # FIXED: Use .data inside no_grad to avoid RuntimeError
    with torch.no_grad():
        # Weak areas split (become weaker/more sensitive)
        # Strong areas merge (become harder)
        mask_weak = plate.elasticity.data < 1.0
        plate.elasticity.data[mask_weak] *= 0.9
        plate.elasticity.data[~mask_weak] *= 1.1

        # Safety clamp
        plate.elasticity.data.clamp_(0.1, 15.0)
...
def run_experiment(mode):
    ...
    # Load previous state (Continuity of Identity)
    if os.path.exists(CHECKPOINT_FILE):
        print(f"   📂 Loading organism from {CHECKPOINT_FILE}...")
        # Fixed weights_only warning
